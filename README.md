...
> Mac 通过 USB 控制安卓手机自动跑抖音工作流：搜关键词 → 进视频 → 关注 → 发私信 → 滑下一条。带 Web 控制台、多工作流、实时日志、截图回放。
...
| 文件 | 作用 |
|------|------|
| `rpa_mvp.py` | RPA 引擎，按 yaml 描述执行 ADB 动作 |
| `dashboard.py` | Flask Web 控制台（端口 8003） |
| `workflows/*.yaml` | 工作流定义，一个文件一个独立场景 |
| `templates/index.html` + `static/dashboard.{css,js}` | 控制台前端，纯 HTML/JS 无框架 |
| `outputs/run_*/` | 每次运行的截图、UI dump、发送记录 |
| `assets/` | 图像匹配模板（`tap_image` 用，目前空） |
...
浏览器打开 **http://localhost:8003** 就能用了。
...
4. 想发多少条 → 改候选数
5. 想换关键词 → 改关键词输入框
6. **手机不用回首页，工作流会自己冷启动抖音**
7. 点 **▶ 启动 workflow**
8. 看右侧日志和下方截图，跑完进度条满

**inspect 按钮**：只截图 + 抓 UI 树，**不点任何东西**。调试新关键词、新 UI 时用。
...
workflow:
  ...
  open_search: [...]            # 从首页到搜索页的动作
  submit_search: [...]          # 提交搜索 + 进第一个视频
  per_item: [...]               # 每个候选博主完整闭环
drafts:
  dm_template: "您好..."        # 固定话术
```
...
| action | 参数 | 说明 |
|--------|------|------|
| `wait` | `seconds: 1.5` | 死等 N 秒 |
| `tap_text` | `value: "搜索"` `clickable: true` | 找文字匹配的元素并点（用 adb input tap） |
| `tap_desc` | `value: "关注"` `clickable: true` | 找 content-desc 元素 |
| `tap_xy` | `x: 0.5` `y: 0.3` | 比例坐标点击 |
| `input_text` | `value: "..."` 或 `value_from: dm_template` | 输入文字（用 ADBKeyboard 支持中文） |
| `press` | `key: back` | back / home / enter |
| `swipe` | `start: [0.5, 0.8]` `end: [0.5, 0.2]` `duration: 0.4` | 比例滑动 |
| `screenshot` | `name: first_video` | 存截图到 outputs/ |
...
| `tap_text "发私信"` 失败 | 博主主页加载慢 → `obj.exists` timeout 调到 3s（默认） |
| `tap_text "视频"` 失败 | 还在搜索建议页，没真搜索 → 用 `tap_text "搜索"` 提交 |
| `input_text` 报 `null object reference` | 上一步没真进入输入框 → 检查 tap 是否命中 |
| 视频卡片 UI 树没文字 | 抖音 SurfaceView 渲染，只能用 `tap_xy` |
| 莫名其妙跳到上次 Activity | u2.app_start 在抖音上会 restore，必须用 `adb am force-stop` + `monkey`（已修） |
| u2 的 `click()` 没反应 | 抖音视频流吸收合成事件，必须用 `adb shell input tap`（已修） |
...
- **当前默认全自动发送**，不二次确认
- 私信节奏：~22 秒/条（含 splash + 搜索 + 关注 + 发送 + back + swipe）
...
│   └── dashboard.js
├── workflows/
│   ├── douyin-dm.yaml              # 默认工作流：抖音私信
│   ├── _template.yaml              # 模板，给新工作流抄
│   └── README.md                   # 写新工作流的指南
├── assets/                         # tap_image 模板，目前空
├── .venv/
└── outputs/                        # 运行产物（截图 / UI dump）
    └── run_YYYYMMDD_HHMMSS/
...
- [ADBKeyboard](https://github.com/senzhk/ADBKeyBoard) — 中文输入（已经装在手机上）
- [Flask](https://flask.palletsprojects.com/) + SSE — 控制台后端
