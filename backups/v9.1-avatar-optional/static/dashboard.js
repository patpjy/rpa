...
els.btnWorkflow.addEventListener("click", async () => {
  const body = {
    ...
    dm_template: els.dmTemplate.value.trim() || undefined,
  };
...
function applyWorkflow() {
  ...
  if (!opt) return;
  // 持久化优先: 本地存过就用本地;没存过用 yaml 默认值
  els.keyword.value = localStorage.getItem("rpa.keyword") || opt.dataset.keyword || "";
  els.maxItems.value = localStorage.getItem("rpa.maxItems") || opt.dataset.maxItems || 1;
  els.dmTemplate.value = localStorage.getItem("rpa.dmTemplate") || opt.dataset.dmTemplate || "";
  els.workflowDesc.textContent = opt.dataset.description || "";
  els.configBadge.textContent = `工作流: ${opt.value}`;
}

// 输入即存,关浏览器/换电脑都不丢
els.keyword.addEventListener("input", () => localStorage.setItem("rpa.keyword", els.keyword.value));
els.maxItems.addEventListener("input", () => localStorage.setItem("rpa.maxItems", els.maxItems.value));
els.dmTemplate.addEventListener("input", () => localStorage.setItem("rpa.dmTemplate", els.dmTemplate.value));
